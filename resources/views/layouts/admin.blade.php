...
<head>
    ...
    <script nonce="{{ request()->attributes->get('csp_nonce') }}">
        ...
        try {
            if (localStorage.getItem('adminSidebar') === 'closed' || window.matchMedia('(max-width: 1023px)').matches) {
...
<body class="admin-shell min-h-screen font-sans text-slate-800 antialiased">
    <div class="admin-desktop-content min-h-screen" data-admin-desktop-content>
        ...
        <aside
            id="admin-sidebar"
            class="admin-sidebar fixed left-0 top-0 z-40 flex h-screen w-72 flex-col overflow-y-auto bg-maroon-950 text-cream-100"
            aria-label="Panel navigasi admin"
        >
            @php
                $adminLogoUrl = $site->logo_path
                    ? \Illuminate\Support\Facades\Storage::url($site->logo_path)
                    : asset('assets/images/branding/logo-fpk.webp');
                $adminSection = match (true) {
                    ...
                    default => 'Panel Administrator',
                };
            @endphp
            <div class="flex items-center gap-3 border-b border-white/10 px-5 py-5">
                <a href="{{ route('admin.dashboard') }}" class="group flex min-w-0 flex-1 items-center gap-3">
                    <span class="grid h-11 w-11 flex-none place-items-center overflow-hidden rounded-full bg-cream-50 ring-2 ring-gold-400/25">
                    ...
                    <span class="min-w-0">
                        ...
                        <span class="block text-[10px] font-semibold uppercase tracking-[0.18em] text-gold-300/65">Ruang Kerja Admin</span>
                    </span>
                </a>
            </div>
        ...
        <button type="button" class="admin-sidebar-backdrop" data-admin-nav-toggle aria-label="Tutup navigasi"></button>

        <div class="admin-content-col flex min-h-screen flex-col">
            <header class="admin-topbar sticky top-0 z-20 flex min-h-18 items-center justify-between gap-2 border-b border-slate-200/80 bg-white/90 px-3 py-3 backdrop-blur-xl sm:gap-4 sm:px-5 lg:px-8">
                <div class="flex min-w-0 flex-1 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="hidden text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400 sm:block">{{ $adminSection }}</p>
                        <h1 class="truncate font-display text-base font-semibold leading-tight text-slate-900 sm:text-xl">@yield('heading', 'Panel Admin')</h1>
                    </div>
                </div>
                ...
                <div class="flex flex-none items-center gap-2 sm:gap-3">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="admin-button admin-button-secondary px-3! py-2!" aria-label="Keluar dari admin" title="Keluar dari admin">
            ...
            <main class="flex-1 px-3 py-5 sm:px-5 sm:py-6 lg:px-8 lg:py-7">
                <div class="mx-auto max-w-7xl space-y-5 sm:space-y-6" data-admin-content data-motion-children>
                    <x-admin.flash />
                    @yield('content')
                </div>
            </main>
    ...
    <script nonce="{{ request()->attributes->get('csp_nonce') }}">
        (function () {
            ...
            document.addEventListener('click', function (event) {
                ...
                var collapsed = root.classList.toggle('admin-nav-collapsed');
                root.classList.toggle('admin-nav-open', ! collapsed);
                try {
                    localStorage.setItem('adminSidebar', collapsed ? 'closed' : 'open');
                } catch (e) {}
            });
        })();
    </script>
